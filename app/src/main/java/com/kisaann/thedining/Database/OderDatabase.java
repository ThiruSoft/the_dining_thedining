...
public class OderDatabase extends SQLiteAssetHelper {
    private static final String DB_NAME = "dinningapp_cartDBB.db";
    private static final int B_VER = 1;

    public OderDatabase(Context context) {
    ...
    public boolean checkFoodExists(String foodId, String userPhone){
        ...
        Cursor cursor = null;
        String SQLQuery = String.format("SELECT * From PlaceOderDetails WHERE UserPhone='%s' AND " +
    ...
    public List<Order> getOrderCart(String userPhone){
        SQLiteDatabase db = getReadableDatabase();
        ...
        String sqlTable = "PlaceOderDetails";
    ...
    public void addToOrderCart(Order order){
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("INSERT OR REPLACE INTO PlaceOderDetails" +
    ...
    public void cleanOrderCart(String userPhone){
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("DELETE FROM PlaceOderDetails WHERE UserPhone='%s'",userPhone);
        db.execSQL(query);
    }


    public int getCountOrderCart(String userPhone) {
        int count = 0;
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("SELECT COUNT(*) FROM PlaceOderDetails Where UserPhone='%s'", userPhone);
    ...
    public void updateOrderCart(Order order) {
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("UPDATE PlaceOderDetails SET Quantity= '%s' WHERE UserPhone = " +
                        "'%s' AND ProductId= '%s'",
                order.getQuantity(),order.getUserPhone(),order.getProductId());
        db.execSQL(query);
    }

    public void increaseOrderCart(String userPhone, String foodId) {
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("UPDATE PlaceOderDetails SET Quantity= Quantity+1 WHERE UserPhone" +
                        " = " +
                        "'%s' AND ProductId= '%s'",
                userPhone,foodId);
        db.execSQL(query);
    ...
    public void removeFromOrderCart(String productId, String phone) {
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("DELETE FROM PlaceOderDetails WHERE UserPhone='%s' and " +
                        "ProductId='%s'",
                phone,productId);
        db.execSQL(query);
    }
