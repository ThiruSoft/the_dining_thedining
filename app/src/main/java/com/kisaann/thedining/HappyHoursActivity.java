...
import com.kisaann.thedining.ViewHolders.RestaurantHappyHoursHolder;
...
public class HappyHoursActivity extends AppCompatActivity {

    String restaurantMedId;
    RecyclerView recycler_menu;
    RecyclerView recyclerView;
    DatabaseReference categories;
    FirebaseRecyclerAdapter<Category, RestaurantHappyHoursHolder> restaurantHappyAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Happy127");
        ...
        txt_viewCart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (cartItems.size() > 0) {
                    startActivity(new Intent(HappyHoursActivity.this, CartItemsActivity.class));
                }else {
                    Toast.makeText(HappyHoursActivity.this, "Your cart is empty!!", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    private void displayMenuDialog(String itemName) {
        ...
        if (itemName.equalsIgnoreCase("food")){
            for (int i = 0; i < menuNames.size(); i++) {
                ...
                txt_menuName.setText(menuNames.get(i).getMenuName());
                //  txt_menuCount.setText("("+menuNames.get(i).getMenuCount()+" Items)");
                layout.setId(i);
                int finalI = i;
                layout.setOnClickListener(view -> {
                    int pos = view.getId();
                    int position = Integer.parseInt(menuNames.get(pos).getMenuCount());
    ...
    private void readCategories() {
        ...
        Query reference = categories.orderByChild("offerType").equalTo("Offer");
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()){
                    ...
                    if (categoryPos == 0){
                        if (category.getType() != null && !category.getType().isEmpty()){
                            if (category.getType().equalsIgnoreCase("Wine")){
                                        ...
                                        && category.getTimeTo() != null && !category.getTimeTo().isEmpty())
                                {
                                    ...
                                    if (hour >= fromTime && hour < toTime) {
                                        ...
                                        barMenuNames.add(new MenuNamesModel(category.getName(),""+categoryPos));
                                    }
                                }
                            }else {
                                if (category.getTimeFrom() != null && !category.getTimeFrom().isEmpty()
                                        && category.getTimeTo() != null && !category.getTimeTo().isEmpty())
                                {
                                    ...
                                    if (hour >= fromTime && hour < toTime) {
                                        Log.e("condition", "true");
                                        menuNames.add(new MenuNamesModel(category.getName(),""+categoryPos));
                                    }
                                }
    ...
    private void loadRestaurantMenuData() {
        if (Utility.isNetworkAvailable(getBaseContext())) {
            ...
            Query listFoodByCategoryId = categories.orderByChild("offerType").equalTo("Offer");
            FirebaseRecyclerOptions<Category> options = new FirebaseRecyclerOptions.Builder<Category>()
                    .setQuery(listFoodByCategoryId, Category.class)
                    .build();
            /* menuNames.clear();*/

            restaurantHappyAdapter = new FirebaseRecyclerAdapter<Category, RestaurantHappyHoursHolder>(options) {
                @Override
                protected void onBindViewHolder(@NonNull RestaurantHappyHoursHolder viewHolder, int position, @NonNull Category model) {
                                ...
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    ...
                                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                                        ...
                                        btn_addToCart.setOnClickListener(view -> {
                                            if (foodModel.getAvailability() != null && !foodModel.getAvailability().isEmpty()) {
                                                if (foodModel.getAvailability().equalsIgnoreCase("Yes")) {
                                                    ...
                                                    Toast.makeText(HappyHoursActivity.this, "Not Available", Toast.LENGTH_SHORT).show();
                                                }
                                            }
                                        });
                                        ...
                                        img_itemImage.setOnClickListener(view -> {
                                            Intent foodDetails = new Intent(HappyHoursActivity.this, FoodDetails.class);
                ...
                @NonNull
                @Override
                public RestaurantHappyHoursHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                    View itemView = LayoutInflater.from(parent.getContext())
                            .inflate(R.layout.menu_cate_item_layout, parent, false);

                    return new RestaurantHappyHoursHolder(itemView);
                }
            };
            restaurantHappyAdapter.startListening();
          //  restaurantHappyAdapter.notifyDataSetChanged();
            recyclerView.setAdapter(restaurantHappyAdapter);

        }else {

            Toast.makeText(HappyHoursActivity.this, "Please check your network connection. ", Toast.LENGTH_LONG).show();
        }
    ...
    protected void onResume() {
        ...
        if (restaurantHappyAdapter != null){
            restaurantHappyAdapter.startListening();
        }
    }
    ...
    protected void onStop() {
        super.onStop();
        if (restaurantHappyAdapter != null){
            restaurantHappyAdapter.stopListening();
        }
    }
}
