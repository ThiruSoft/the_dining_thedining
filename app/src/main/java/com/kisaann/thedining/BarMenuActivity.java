...
import com.kisaann.thedining.ViewHolders.BarMenuHolder;
...
public class BarMenuActivity extends AppCompatActivity {
    ...
    FirebaseRecyclerAdapter<Category, BarMenuHolder> barMenuAdapter;
...
//        layoutManager.startSmoothScroll(smoothScroller);
        ...
        txt_viewCart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (cartItems.size() > 0) {
                    Intent intent = new Intent(BarMenuActivity.this, CartItemsActivity.class);
                    startActivity(intent);
                }else {
                    Toast.makeText(BarMenuActivity.this, "Your cart is empty!!", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
        ...
        if (itemName.equalsIgnoreCase("food")){
            ...
            for (int i = 0; i < barMenuNames.size(); i++) {
                ...
                layout.setOnClickListener(view -> {
                    ...
                    Log.e("position",""+position);
                  //  smoothScroller.smoothScrollToPosition(position);
    ...
    private void readCategories() {
        ...
        Query reference = categories.orderByChild("categoryType").equalTo("Non-OfferWine");
    ...
    private void loadRestaurantMenuData() {
        if (Utility.isNetworkAvailable(getBaseContext())) {
            ...
            Query listFoodByCategoryId = categories.orderByChild("categoryType").equalTo("Non-OfferWine");
            FirebaseRecyclerOptions<Category> options = new FirebaseRecyclerOptions.Builder<Category>()
                    .setQuery(listFoodByCategoryId, Category.class)
                    .build();
            /* menuNames.clear();*/

            barMenuAdapter = new FirebaseRecyclerAdapter<Category, BarMenuHolder>(options) {
                @Override
                protected void onBindViewHolder(@NonNull BarMenuHolder viewHolder, int position, @NonNull Category model) {
                                ...
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    ...
                                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                                        ...
                                        btn_addToCart.setOnClickListener(view -> {
                                            if (foodModel.getAvailability() != null && !foodModel.getAvailability().isEmpty()) {
                                                if (foodModel.getAvailability().equalsIgnoreCase("Yes")) {
                                                    ...
                                                    Toast.makeText(BarMenuActivity.this, "Not Available", Toast.LENGTH_SHORT).show();
                                                }
                                            }
                                        });
                                        ...
                                        img_itemImage.setOnClickListener(view -> {
                                            Intent foodDetails = new Intent(BarMenuActivity.this, FoodDetails.class);
                ...
                @NonNull
                @Override
                public BarMenuHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                    View itemView = LayoutInflater.from(parent.getContext())
                            .inflate(R.layout.menu_cate_item_layout, parent, false);

                    return new BarMenuHolder(itemView);
                }
            };
            barMenuAdapter.startListening();
          //  barMenuAdapter.notifyDataSetChanged();
            recyclerView.setAdapter(barMenuAdapter);

        }else {

            Toast.makeText(BarMenuActivity.this, "Please check your network connection. ", Toast.LENGTH_LONG).show();
        }
